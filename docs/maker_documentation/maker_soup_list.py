...
def find_unknown_dependencies(requirements, imports, standard_libs, excluded_files):
    ...
    unknown_imports = [imp for imp in imports if imp not in requirements and imp not in standard_libs 
                       and imp not in excluded_files and imp not in local_modules 
                       and imp not in excluded_dependencies]
...
def generate_soup_list(requirements, unknown_dependencies):
    ...
    for item in soup_list:
        if item['Package Name'] not in seen:
